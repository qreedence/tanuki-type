...
export default function AnimatedTabs({
  ...
  const baseContainerStyles = cn(
    ...
    variant === "pill" && "gap-1 rounded-xl border border-border bg-secondary p-1 [transform:translateZ(0)]",
    variant === "segment" && "gap-0 rounded-lg bg-muted p-1"
  );

  const getTabStyles = (isActive: boolean) =>
    cn(
      "relative z-10 flex min-h-9 min-w-10 items-center justify-center gap-2 border-transparent px-4 py-2 font-medium text-sm transition-colors",
  ...
  const getIndicatorStyles = () =>
    cn(
      "absolute border-transparent [transform:translateZ(0)] [backface-visibility:hidden] [-webkit-backface-visibility:hidden]",
      variant === "underline" && "right-0 -bottom-px left-0 h-0.5 bg-brand",
      variant === "pill" &&
        "inset-0 rounded-lg bg-primary",
      variant === "segment" &&
      ...
      {tabs.map((tab, index) => {
        ...
        return (
          ...
          >
            ...
            {tab.label && <span className="relative z-10">{tab.label}</span>}
          </button>
        );
      })}
    </div>
